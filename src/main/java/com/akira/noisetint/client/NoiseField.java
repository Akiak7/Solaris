...
private static final double MAIN_SCALE = 1.0 / 2048.0;   // larger regions
private static final double WARP_SCALE = 1.0 / 1024.0;
private static final double WARP_AMPLITUDE = 0.0;

private static final float HUE_DEG_AMPL = 60f;   // was 14
private static final float SAT_MUL_AMPL = 0.24f; // was 0.06
private static final float VAL_MUL_AMPL = 0.24f; // was 0.08
    ...
    public static float[] sampleTint(int wx, int wz) {
        ...
        // soften each channel for more gradual changes
        nx0 = soften(nx0);
        nx1 = soften(nx1);
        nx2 = soften(nx2);
    ...
    private static double soften(double n) {
        // n in [-1,1] -> t in [0,1]
        double t = 0.5 * (n + 1.0);
        // cubic smoothstep; flattens extremes and eases transitions
        t = t * t * (3.0 - 2.0 * t);
        // back to [-1,1]
        return t * 2.0 - 1.0;
    }
